...
function IframeWithoutMask(url,width,height,title,maxBtn,minBtn,isParent,handler){ 
	if(ymPrompt_reload) handler = handler || WinClose;
	else handler = handler || null;
	if(isParent){
		var clientWidth = top.getWindowWidth();
		...
		window.top.ymPrompt.win({message:url,width:width,height:height,title:title,handler:handler,maxBtn:maxBtn,minBtn:minBtn,iframe:true,showMask:false});
	}else{ 
		var clientWidth = getWindowWidth();
		var clientHeight = getWindowHeight();
		if(width > clientWidth || height > clientHeight){
		...
		ymPrompt.win({message:url,width:width,height:height,title:title,handler:handler,maxBtn:maxBtn,minBtn:minBtn,iframe:true,showMask:false});
	} 
} 
